...
for (f in data_files){
  ...
  info = strsplit(f, '_')[[1]]
  tmp$pNum = as.numeric(gsub(pattern = 'p', replacement = '', x = info[3]))
  tmp$group = as.numeric(gsub(pattern = 'g', replacement = '', x = info[4]))
...
prec_dat$fullObs = as.integer(!with(prec_dat, recallMissed | notMoved | outBounds | tafcACC!=1))

prec_dat_sandt = prec_dat # study and test events

prec_dat = subset(prec_dat, eventType != "study")
...
prec_dat_sandt_cle = subset(prec_dat_sandt, !(ID %in% to_remove))

mean(prec_dat$fullObs)
mean(prec_dat$recallMissed)

# write 'cleaned' file
write.csv(prec_dat, file = "exp2_cleaned.csv", row.names = F)
write.csv(prec_dat_sandt_cle, file = "exp2_full-cleaned.csv", row.names = F)
